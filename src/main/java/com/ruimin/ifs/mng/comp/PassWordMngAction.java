...
import com.ruimin.ifs.mng.process.service.PassWordMngService;
import com.ruimin.ifs.po.TblSysParam;

@SnowDoc(desc = "系统参数设置")
@ActionResource
public class PassWordMngAction extends SnowAction {
	...
	public PageResult queryAll(QueryParamBean queryBean) throws SnowException {
		...
		return PassWordMngService.getInstance().queryList(queryParamId, queryOprcode1, queryBean.getPage());
	}

	@Action(propagation = Propagation.REQUIRED)
	@SnowDoc(desc = "系统参数修改")
	public void saveOrUpdate(Map<String, UpdateRequestBean> updateMap) throws SnowException {
		UpdateRequestBean reqBean = updateMap.get("PassWordMng");
		TblSysParam tlp = new TblSysParam();
		PassWordMngService sps = ContextUtil.getSingleService(PassWordMngService.class);
	...
	public void delSysParams(Map<String, UpdateRequestBean> updateMap) throws SnowException {
		...
		PassWordMngService.getInstance().delSysParams(paramId, magicId);
	...
	public void addSysParams(Map<String, UpdateRequestBean> updateMap) throws SnowException {
		 ...
		 * PassWordMngService.getInstance().queryCityByCtCode(tlp.getCtCode());
		 * if (count>0) {
		 * SnowExceptionUtil.throwWarnException("WEB_E0046",tlp.getCtCode()); }
		 */
		PassWordMngService.getInstance().addSysParams(tlp);
	}
