...
import com.ruimin.ifs.pmp.report.process.service.MchtTxnCountDailyService;

@ActionResource
@SnowDoc(desc = "商户交易统计日报action")
public class MchtTxnCountDailyAction extends SnowAction {
	/**
	 * 
	 ...
	 */
	@Action
	@SnowDoc(desc = "商户交易统计日报信息分页查询")
	public PageResult pageQuery(QueryParamBean queryBean) throws SnowException {
		String qDateStlmStart = queryBean.getParameter("qDateStlmStart");
		String qDateStlmEnd = queryBean.getParameter("qDateStlmEnd");
		...
		return MchtTxnCountDailyService.getInstance().pageQuery(qDateStlmStart, qDateStlmEnd, qMchtInfo, qMchtOrg, queryBean.getPage());

	}
	 @Action
     @SnowDoc(desc = "商户交易统计日报表下载")
     public void doutAgent(FormRequestBean formRequestBean) throws SnowException, UnsupportedEncodingException {
	   //判断用户是否有权限下载报表
         ...
         String str=UserAuthorityDownService.getInstance().qTlrRole(tlrno, "1015001");
         if(tlrno.equals(str)){              
         }else{
	     ...
	     // 获取查询条件的值
            String qDateStlmStart = formRequestBean.getParameter("qDateStlmStart");//统计月份(起始)
            String qDateStlmEnd = formRequestBean.getParameter("qDateStlmEnd");//统计月份(截止)
         ...
         // 如果查询选择的机构为空，则默认查询本机构和本机构下的子级机构商户
            ...
            try {
                /** 获取Title **/
                String title = "商户交易统计日报表下载";
                /** 获取路径 **/
                String relPath = "/report/jasper/mchtTxnCountDaily.jasper";
                /** 获取PageHeader **/
                Map<String, Object> pageHeader = new HashMap<String, Object>();
                pageHeader.put("qDateStlmStart", qDateStlmStart);
                pageHeader.put("qDateStlmEnd", qDateStlmEnd);
                pageHeader.put("qMchtInfo", qMchtInfo);
                /** 获取Detail **/
                List<Object> detail = MchtTxnCountDailyService.getInstance().doutAgent(qDateStlmStart, qDateStlmEnd, qMchtInfo, qMchtOrg);
                qMchtOrg = MchtTxnCountDailyService.getInstance().queryMchtOrg(qMchtOrg);
