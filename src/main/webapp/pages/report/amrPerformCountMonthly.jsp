...
<!------------------------------------------------------------------------------------------------- snow自定义标签---------------------------------------------------------------------------------------------------->
<snow:page title="业务员绩效统计月报表">
	<snow:dataset id="profitCountMonthly"
		path="com.ruimin.ifs.pmp.report.dataset.amrPerformCountMonthly"
		submitMode="current" init="true"></snow:dataset>
	<!-- 查询条件 -->
	<snow:query label="请输入查询条件" id="queryId" dataset="profitCountMonthly"
		fieldstr="qStlmMonthStart,qStlmMonthEnd,qMchtAmrNo,qMchtAmrName"></snow:query>
	<!-- 查询结果集分页显示 -->
	<snow:grid dataset="profitCountMonthly" id="gridId" height="99%" fieldstr="stlmMonth[105],mchtAmrNo,mchtAmrName,sigedContMchtCount,txnMchtCount,stlmDateTime"
	...
	function btnDownload_onClickCheck(){
		if(profitCountMonthly_dataset.length==0){
        	 ...
        	 var qMchtAmrNo = profitCountMonthly_interface_dataset.getValue("qMchtAmrNo");//代理商编号
        	 var qMchtAmrName = profitCountMonthly_interface_dataset.getValue("qMchtAmrName");//代理商名称
        	 if(qStlmMonthStart){
        	 ...
        	 /*****进入后台*****/
        	 var url = '<snow:url flowId="com.ruimin.ifs.pmp.report.comp.AmrPerformCountMonthlyAction:doutAgent" />' + "&qStlmMonthStart="+qStlmMonthStart + "&qStlmMonthEnd="+qStlmMonthEnd + "&qMchtAmrNo="+qMchtAmrNo + "&qMchtAmrName="+qMchtAmrName;
        	 url = encodeURI(url);
        	 url = encodeURI(url);
