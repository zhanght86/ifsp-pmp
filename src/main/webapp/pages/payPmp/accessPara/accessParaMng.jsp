...
<snow:page title="接入系统参数配置">
	...
	<snow:query label="查询条件" id="queryId" collapsible="false"
		...
		fieldstr="qCertifiId,qcertifiStatus"></snow:query>

	<!-- 3.主表单 -->
	<snow:grid dataset="MchtAccessPara" height="99%" label="接入系统参数信息"
		fitcolumns="true" id="gridId"
		fieldstr="certifiBl[180],certifiDate[180],certifiEndDate[180],certifiStatus[180],certifiType[180],opr[180]"
		paginationbar="btnAdd,btnUpt,btnStatus"></snow:grid>

	<!-- 3.新增 -->
	<snow:window id="windowAddId" title="接入系统参数--> 新增" width="850"
		modal="true" closable="true" buttons="btnAddSave">
		<snow:form id="baseInfo1" dataset="MchtAccessPara" border="false" label=""
			fieldstr="certifiBl,certifiStatus,certifiDate,certifiEndDate,certifiPasswd">
		</snow:form>
		<snow:button id="btnAddSave" dataset="MchtAccessPara" hidden="true"></snow:button>
	</snow:window>

	<!-- 4.修改 -->
	<snow:window id="windowUpdateId" title="接入系统参数--> 修改" width="850"
		modal="true" closable="true" buttons="btnUpdSave">
		<snow:form id="baseInfo2" dataset="MchtAccessPara" border="false" label=""
			fieldstr="certifiBl,certifiDate,certifiEndDate,certifiStatus,certifiPasswd">
		</snow:form>
	...
	<snow:window id="winDetail" title="详情信息" modal="true" closable="true">
		...
		<snow:form id="MchtCertDetail" label="基本信息" dataset="MchtAccessPara"
			fieldstr="certifiBl,certifiType,certifiDate,certifiEndDate,certifiPasswd">
		</snow:form>
	</snow:window>
	...
	<script>
		...
		function checkField(flag) {
			var certifiBl = MchtAccessPara_dataset.getValue("certifiBl");
...
//		}
		...
		function btnAdd_onClick() {
			...
			MchtAccessPara_dataset.setFieldReadOnly("certifiBl", false); 
...
//			});
		...
		function btnUpt_onClick() {
			MchtAccessPara_dataset.setFieldReadOnly("certifiBl", true);
			MchtAccessPara_dataset.setFieldReadOnly("certifiPasswd", true);
			MchtAccessPara_dataset.setFieldReadOnly("certifiDate", false);
			MchtAccessPara_dataset.setFieldReadOnly("certifiEndDate", false);
			MchtAccessPara_dataset.setFieldReadOnly("certifiStatus", false);
		...
		function btnStatus_onClick() {
			...
			var msg = "";
			if (dataState == "01") {
				msg = "是否要将该接入系统参数状态修改为【启用】?";
				dataState = "00";
			} else {
				msg = "是否要将该接入系统参数状态修改为【停用】?";
				dataState = "01";
			}
