...
//		width : 60,
	...
	$.ligerui.controls.Button.ligerExtend($.ligerui.controls.Input, {
		...
		_render : function() {
			...
			g.button = $(g.element);
			g.button.addClass("l-button" + (p.plain ? " l-button-plain" : ""));// zzg
			g.button.append('<div class="l-button-l"></div><div class="l-button-r"></div><span>' + p.text + '</span>');
			g.button.hover(function() {
				if (p.disabled)
					return;
				p.plain && g.button.removeClass("l-button-plain");
				g.button.addClass("l-button-over");
			}, function() {
				if (p.disabled)
					return;
				g.button.removeClass("l-button-over");
				p.plain && g.button.addClass("l-button-plain");
			});
...
//				g.button.append(UIUtil.iconHTML(url));
				$("span", this.button).before(UIUtil.iconHTML(url));
			}
		},
