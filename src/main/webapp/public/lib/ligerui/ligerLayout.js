	...
	$.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
		...
		_init : function() {
			...
			if (p.InWindow != null && p.inWindow == null)
				p.inWindow = p.InWindow; // 旧版本命名错误纠正
		},
		_render : function() {
			var g = this, p = this.options;
			...
			// top
			var topcontent = $("> div > div[position=top]", g.layout);
			if (topcontent.length > 0) {
				g.top = topcontent.parent();
				g.top.content = topcontent;
			...
			// bottom
			var bottomcontent = $("> div > div[position=bottom]", g.layout);
			if (bottomcontent.length > 0) {
				g.bottom = bottomcontent.parent();
				g.bottom.content = bottomcontent;
			...
			// left
			var leftcontent = $("> div > div[position=left]", g.layout);
			if (leftcontent.length > 0) {
				g.left = leftcontent.parent();
			...
			// center
			var centercontent = $("> div > div[position=center]", g.layout);
			if (centercontent.length > 0) {
				g.center = centercontent.parent();
				g.center.content = centercontent;
			...
			// right
			var rightcontent = $("> div > div[position=right]", g.layout);
			if (rightcontent.length > 0) {
				g.right = rightcontent.parent();
		...
		_onResize : function() {
			...
			if (g.center) {
				...
				g.center.content.height(g.middleHeight - (g.center.header ? g.center.header.height() : 0));// zzg
				g._updateCenterBottom(true);
			}
			if (g.left) {
				g.leftCollapse.height(g.middleHeight);
				g.left.height(g.middleHeight);
				g.left.content.height(g.middleHeight - (g.left.header ? g.left.header.height() : 0));// zzg
			}
			if (g.right) {
				...
				g.right.content.height(g.middleHeight - (g.right.header ? g.right.header.height() : 0));// zzg
			}
			if (g.bottom) {
