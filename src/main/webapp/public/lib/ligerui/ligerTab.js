	...
	$.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render : function() {
			...
			if (p.contextmenu && $.ligerMenu) {
				g.tab.menu = $.ligerMenu({
					...
					items : [ {
						...
						text : p.closeLeftMessage,
						id : 'closeleft',
						click : function() {
							g._menuItemClick.apply(g, arguments);
						}
					}, {
						text : p.closeRightMessage,
						id : 'closeright',
						click : function() {
							g._menuItemClick.apply(g, arguments);
						}
			...
			$("> div", g.tab.content).each(function(i, box) {
				...
				if (tabid == undefined) {
					tabid = contentitem.attr("id") || g.getNewTabid();
				...
				if (showClose=="true") {//zzg
					li.append("<div class='l-tab-links-item-close'></div>");
				}
				$("> ul", g.tab.links).append(li);

				var iconCls = contentitem.attr("icon");
				if (iconCls) {
					g.setIcon(tabid, iconCls);
				}
		...
		moveToPrevTabItem : function(tabid) {
			var g = this, p = this.options;
			var tabItems = $("> li", g.tab.links.ul), nextBtn = $(".l-tab-links-right", g.tab), prevBtn = $(".l-tab-links-left", g.tab);
		...
		setClosable : function(tabid, closable) {//zzg add
			var g = this, p = this.options;
		...
		setIcon : function(tabid, icon) {
			$("li[tabid=" + tabid + "] a", this.tab.links.ul).attr("style","padding-left:5px;");
			$("li[tabid=" + tabid + "] .l-tab-links-item-left", this.tab.links.ul).html(UIUtil.iconHTML(icon));
		},
		// 设置是否可用
		setEnable : function(tabid, isEnable) {
			var g = this, p = this.options;
			var tabitem = $("li[tabid=" + tabid + "]", g.tab.links.ul);
			// false不可用
			if (!isEnable) {
				if ($(".l-tab-links-item-disabled", tabitem).length == 0){
					tabitem.append("<div class='l-tab-links-item-disabled'></div>");
				}
				tabs.setClosable(false);
			} else {
				$(".l-tab-links-item-disabled", tabitem).remove();
		...
		selectTabItem : function(tabid) {
			var g = this, p = this.options;
			if (g.inited && g.trigger('beforeSelectTabItem', [ tabid, g.selectedTabId ]) == false)
				return false;
			var oldtabid = g.selectedTabId;
			g.selectedTabId = tabid;
			...
			g.trigger('afterSelectTabItem', [ tabid, oldtabid ]);
		},
		// 移动到最后一个tab
		...
		removeLeft : function(tabid, compel) {
			var g = this, p = this.options;
			var tabidlist = [];
			$("> li[tabid=" + tabid + "]", g.tab.links.ul).prevAll().each(function() {
		...
		removeRight : function(tabid, compel) {
			var g = this, p = this.options;
			var tabidlist = [];
			$("> li[tabid=" + tabid + "]", g.tab.links.ul).nextAll().each(function() {
				if ($(".l-tab-links-item-close", this).length > 0) {
					tabidlist.push($(this).attr("tabid"));
				}
			});
		...
		reload : function(tabid, newUrl) {
			...
			var iframeloading = $(".l-tab-loading:first", contentitem);
			var iframe = $("iframe:first", contentitem);
			var url = newUrl || $(iframe).attr("src");
			iframeloading.show();
		...
		onResize : function() {
			...
			if (g.tab.parent()[0].tagName.toLowerCase() == "body") {
				...
				g.height = p.heightDiff + windowHeight * parseFloat(p.height) * 0.01;//zzg g.height->p.height
			} else {
