...
(function($) {
	...
	$.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render : function() {
			...
			// 生成ligerui表单样式
			/*
			 * zzg $("input,select,textarea", jform).each(function() {
		...
		_initFieldsValidate : function(e) {
			...
			if (fields && fields.length) {
				$(fields).each(function(index, field) {
					var name = field.name, readonly = (field.readonly || (field.editor && field.editor.readonly)) ? true : false, txtInputName = (p.prefixID || "") + (field.textField || field.id || field.name);
		...
		_initFieldsHtml : function(e) {
			var g = this, p = this.options;
			var jform = e.panel, fields = e.fields, idPrev = e.idPrev || g.id;
		...
		_createEditors : function(e) {
			...
			$(fields).each(function(fieldIndex, field) {
				var container = $("#" + idPrev + "\\|" + fieldIndex, g.element).get(0), editor = p.editors[field.type], editId = editPrev + fieldIndex;
				if (!container)
					return;
				container = $(container);
		...
		setEnabled : function(arg, isEnabled) {
			...
			function setEnabledInFields(fields, tabIndex) {
				$(fields).each(function(fieldIndex, field) {
					var name = field.name, textField = field.textField, editPrev = tabIndex == null ? "" : "tab" + tabIndex + "_", editor = g.editors[editPrev + fieldIndex];
					if (!editor || !name)
						return;
					if (!editor.editor || !editor.editor.setEnabled)
						return;
		...
		showInvalid : function() {
			...
			if (!g.validator)
				return;
			var jmessage = $('<div><div class="invalid">' + p.invalidMessage.replace('{errorCount}', g.validator.errorList.length) + '<a class="viewInvalidDetail" href="javascript:void(0)">' + p.detailMessage + '</a></div><div class="invalidDetail" style="display:none;">'
					+ getInvalidInf(g.validator.errorList) + '</div></div>');
		...
		_buliderControlContainer : function(field, fieldIndex, idPrev) {
			var g = this, p = this.options;
			var width = field.width || p.inputWidth, align = field.align || field.textAlign || field.textalign || p.align, out = [], idPrev = idPrev || g.id;
			out.push('<li');
		...
		_setTab : function(tab) {
			...
			for (var i = 0; i < tab.items.length; i++) {
				var tabItem = tab.items[i], jnavItem = $('<li class="ui-state-default ui-corner-top"><a href="javascript:void(0)"></a></li>').appendTo(jtabNav), jcontentItem = $('<div class="ui-tabs-panel ui-widget-content ui-corner-bottom">').appendTo(jtab), idPrev = g.id + "|tdb" + i;
		...
		selectTab : function(index) {
			...
			links.filter("[data-index=" + index + "]").addClass("ui-tabs-selected ui-state-active ui-state-hover");
			links.filter("[data-index!=" + index + "]").removeClass("ui-tabs-selected ui-state-active ui-state-hover");
