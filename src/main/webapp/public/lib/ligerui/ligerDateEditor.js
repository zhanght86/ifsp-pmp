...
(function($) {
	...
	$.ligerui.controls.DateEditor
			.ligerExtend(
					$.ligerui.controls.Input,
					{
						__getType : function() {
							return 'DateEditor';
						},
...
//							g.text.append(g.link);
							...
							dateeditorHTML += "        <div class='l-box-dateeditor-time'><a class='l-box-dateeditor-time-hour'></a>:<a class='l-box-dateeditor-time-minute'></a></div>";
							...
							g.toolbar.time.hour = $(".l-box-dateeditor-time-hour", g.toolbar);// $("<a></a>");
							g.toolbar.time.minute = $(".l-box-dateeditor-time-minute", g.toolbar);// $("<a></a>");
							g.buttons = {
							...
							var nowDate = window._today_date || new Date();
							g.now = {
						...
						updateSelectBoxPosition : function() {
							...
							if (p.absolute) {
								...
								if (Number(g.text.offset().left + g.dateeditor.width()) > contentWidth) {
									g.dateeditor.css({
										left : g.text.offset().left + g.text.outerWidth() - g.dateeditor.width()
									});
								}
							} else {
						...
						_bulidDate : function(dateStr) {
							...
							if (p.showTime) {
								if (tt.m < 0 || tt.m > 59 || tt.h < 0 || tt.h > 23 || tt.s < 0 || tt.s > 59)
									return null;
								d.setHours(tt.h);
								d.setMinutes(tt.m);
								d.setSeconds(tt.s);
						...
						_setValue : function(value) {
							...
							if (typeof value == "object") {
								...
								var newDate = g._bulidDate(value);
								if (!newDate) {
									g._rever();
									return;
								} else {
