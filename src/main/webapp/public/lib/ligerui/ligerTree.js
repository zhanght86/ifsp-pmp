...
(function($) {
	...
	$.ligerDefaults.Tree = {
		...
		/*
		 * 是否展开 1,可以是true/false 2,也可以是数字(层次)N 代表第1层到第N层都是展开的，其他收缩 3,或者是判断函数
		 * 函数参数e(data,level) 返回true/false
		 * 
		 * 优先级没有节点数据的isexpand属性高,并没有delay属性高
		 */
		isExpand : null,
		/*
		 * 是否延迟加载 1,可以是true/false 2,也可以是数字(层次)N 代表第N层延迟加载 3,或者是字符串(Url)
		 * 加载数据的远程地址 4,如果是数组,代表这些层都延迟加载,如[1,2]代表第1、2层延迟加载 5,再是函数(运行时动态获取延迟加载参数)
		 * 函数参数e(data,level),返回true/false或者{url:...,parms:...}
		 * 
		 * 优先级没有节点数据的delay属性高
		 */
		delay : null
	...
	$.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
		...
		// 获取父节点 数据
		getParent : function(_id, level) {
			var g = this;
			var treenode = g.getNodeDom(_id);
			...
			return g._getDataNodeByTreeDataId(g.data, $(parentTreeNode).attr("node-id"));
		},
		// 获取父节点 数据
		getParentId : function(_id) {
			var g = this;
			var treenode = g.getNodeDom(_id);
			var parentTreeNode = g.getParentTreeItem(treenode);
			if (!parentTreeNode)
				return null;
			return $(parentTreeNode).attr("node-id");
		},
		// 获取父节点
		...
		getUnChecked : function() {
			...
			if (!this.options.checkbox)
				return null;
			var nodes = [];
			$(".l-checkbox-unchecked", g.tree).parent().parent("li").each(function() {
				var treedataindex = parseInt($(this).attr("treedataindex"));
				nodes.push({
					target : this,
					data : g._getDataNodeByTreeDataIndex(g.data, treedataindex)
		...
		getRoots : function() {
			...
			$(">li", g.tree).each(function() {
				var n = g._getDataNodeByTreeDataId(g.data, $(this).attr("node-id"));
				node.push(n);
			})
			return node;
		},
		...
		collapse : function(_id) {
			var g = this, p = this.options;
			var treeitem = $(g.getNodeDom(_id));
			var ex = treeitem.find("div.l-body>.l-expandable-open");
			if (ex[0]) {
				ex.click();
			}
		...
		expandTo : function(_id) {
			var g = this, p = this.options;
			var parents = [ _id ];
			if (g.cachedData[_id]) {
				var pid = g.cachedData[_id][p.parentIDFieldName];
				while (g.cachedData[pid]) {
					parents.push(pid);
					pid = g.cachedData[pid][p.parentIDFieldName];
				}
				$(parents).each(function() {
					$("li[node-id='" + this + "']>div.l-body>.l-expandable-close", g.tree).click();
				});
			}

		},
		expand : function(_id, deep) {
			var g = this, p = this.options;
			var treeitem = $(g.getNodeDom(_id));
			var ex = treeitem.find(">div.l-body>.l-expandable-close");
			if (ex[0]) {
				ex.click();
				// if (deep) {
				// while (deep--) {
				// var nextid = treeitem.find(">ul>li").attr("node-id");
				// if (nextid) {
				// g.expand(nextid, deep);
				// } else {
				// break;
				// }
				// }
			...
			// $("li[node-id='" + _id + "']>div.l-body>.l-expandable-close",
			// g.tree).click();
		},
		remoteServer : function(fn) {
			return fn;
		},
		loadData : function(node, url, param, e) {
			...
			// zzg begin
			e.showLoading();
			g.trigger("remoteLoad", [ param, function(data) {
				if (!data)
					return;
				$(node).find(">ul").remove();
				e.hideLoading();
				g.append(node ? node.getAttribute("node-id") : null, data);
		...
		getNodeDom : function(_id) {
			var g = this, p = this.options;
			return $("li[node-id=" + _id + "]", g.tree)[0];
		},
		hide : function(_id) {
			var g = this, p = this.options;
			var treeNode = g.getNodeDom(_id);
		...
		remove : function(_id) {
			...
			var treenodedata = g._getDataNodeByTreeDataId(g.data, _id);
			if (treenodedata)
		...
		update : function(_id, newnodedata) {
			var g = this, p = this.options;
			var domnode = g.getNodeDom(_id);
			// var treedataindex = parseInt($(domnode).attr("treedataindex"));
			var nodedata = g._getDataNodeByTreeDataId(g.data, _id);
			if (nodedata) {
				for ( var attr in newnodedata) {
					...
					} else if (attr == p.iconFieldName) {
						var iconHolder = $("> .l-body > .l-tree-icon", domnode);
						iconHolder.empty().append(UIUtil.iconHTML(newnodedata[attr]));
						if (newnodedata[attr]) {
							iconHolder.addClass("l-tree-icon-none");
						} else {
							iconHolder.removeClass("l-tree-icon-none");
						}
					}
				}
			}
		...
		append : function(_pid, newdata, nearNode, isAfter) {
			var g = this, p = this.options;
			var parentNode = g.getNodeDom(_pid);
		...
		selectNode : function(selectNodeParm) {
			...
			if (typeof (selectNodeParm) == "function") {
				...
				if (p.checkbox) {
					// $(".l-checkbox",
					// treeitembody).removeClass("l-checkbox-unchecked
					// l-checkbox-incomplete").addClass("l-checkbox-checked");
			...
			$("li", g.tree).each(function() {
				...
				if (clause(treenodedata, treedataindex)) {
					...
					if (!g.options.checkbox) {
						g.cancelSelect(treeitem.attr("node-id"));
					}
				}
			});
		},
		checkAll : function() {
			var abort = this.trigger("beforeCheckAll");
			if (abort === false)
				return;
			$(".l-checkbox", this.tree).removeClass("l-checkbox-unchecked l-checkbox-incomplete").addClass("l-checkbox-checked");
			this.trigger("afterCheckAll");
		},
		uncheckAll : function() {
			var abort = this.trigger("beforeUnCheckAll");
			if (abort === false)
				return;
			$(".l-checkbox", this.tree).removeClass("l-checkbox-checked l-checkbox-incomplete").addClass("l-checkbox-unchecked");
			this.trigger("afterUnCheckAll");
		},
		checkNode : function(_id, cascade) {
			var g = this, p = this.options;
			var treeitem = $(g.getNodeDom(_id));
			if (cascade || !$(">div>.l-checkbox", treeitem).hasClass("l-checkbox-checked")) {
				var abort = g.trigger('beforeCheck', [ g.cachedData[_id], treeitem[0] ]);
				if (abort === false)
					return;
				if (cascade) {
					$(".l-checkbox", treeitem).removeClass("l-checkbox-unchecked l-checkbox-incomplete").addClass("l-checkbox-checked");
				} else {
					$(">div>.l-checkbox", treeitem).removeClass("l-checkbox-unchecked l-checkbox-incomplete").addClass("l-checkbox-checked");
				}
				g.trigger('check', [ {
					data : g.cachedData[_id],
					target : treeitem[0]
				} ]);
			}
		},
		unCheckNode : function(_id, cascade) {
			var g = this, p = this.options;
			var treeitem = $(g.getNodeDom(_id));
			if (cascade || !$(">div>.l-checkbox", treeitem).hasClass("l-checkbox-unchecked")) {
				var abort = g.trigger('beforeUnCheck', [ g.cachedData[_id], treeitem[0] ]);
				if (abort === false)
					return;
				if (cascade) {
					$(".l-checkbox", treeitem).removeClass("l-checkbox-checked l-checkbox-incomplete").addClass("l-checkbox-unchecked");
				} else {
					$(">div>.l-checkbox", treeitem).removeClass("l-checkbox-checked l-checkbox-incomplete").addClass("l-checkbox-unchecked");
				}
				g.trigger('unCheck', [ g.cachedData[_id], treeitem[0] ]);
			}
		},
		...
		_getDataNodeByTreeDataId : function(data, _id) {
			var g = this, p = this.options;
			return g.cachedData[_id];
		},
		// 设置数据状态
		...
		_addTreeDataIndexToData : function(data) {
			var g = this, p = this.options;
			g.cachedData = g.cachedData || {};
			$(data).each(function() {
				g.cachedData[this[p.idFieldName]] = this;
		...
		// 获取节点的延迟加载状态,返回true/false (本地模式) 或者是object({url
		// :'...',parms:null})(远程模式)
		...
		_getTreeHTMLByData : function(data, outlineLevel, isLast, isExpand) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (o[p.idFieldName]) {
					treehtmlarr.push('node-id="' + o[p.idFieldName] + '" ');
				}
				if (isExpandCurrent)
				...
				if (g.hasChildren(o)) {
					...
					if (p.parentIcon) {
						...
						treehtmlarr.push('">');
						if (p.iconFieldName && o[p.iconFieldName])
							treehtmlarr.push(UIUtil.iconHTML(o[p.iconFieldName], "fa fa-file-o"));// '<img
						// src="'
						// +
						// o[p.iconFieldName]
						// + '"
		...
		_setTreeEven : function() {
			...
			g.tree.click(function(e) {
				...
				// else
				// treeitem = $(obj);
				treeitem = $(obj).closest("li");
				if (!treeitem)
					return;
				// var treedataindex = parseInt(treeitem.attr("treedataindex"));
				var treenodedata = g._getDataNodeByTreeDataId(g.data, treeitem.attr("node-id"));
				...
				if (!$(obj).hasClass("l-checkbox") && !clickOnTreeItemBtn) {
					if (!treeitem.hasClass("l-unselectable")) {
						var hasSelected = $(">div:first", treeitem).hasClass("l-selected");
						if (hasSelected && p.needCancel) {
				...
				if ($(obj).hasClass("l-checkbox")) {
					if (p.autoCheckboxEven) {
						// 状态：未选中
						if ($(obj).hasClass("l-checkbox-unchecked") || $(obj).hasClass("l-checkbox-incomplete")) {
							var abort = g.trigger('beforeCheck', [ treenodedata, treeitem[0] ]);
							if (abort === false) {
								return;
							}
							$(obj).removeClass("l-checkbox-unchecked l-checkbox-incomplete").addClass("l-checkbox-checked");
						...
						// 状态：选中
							...
							var abort = g.trigger('beforeUnCheck', [ treenodedata, treeitem[0] ]);
							if (abort === false) {
								return;
							}
