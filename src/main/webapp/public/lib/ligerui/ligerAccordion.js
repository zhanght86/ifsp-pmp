...
(function($) {
	...
	$.ligerui.controls.Accordion.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render : function() {
			...
			var selectedIndex = $("[isselected=true]", g.accordion).prevAll().size();
			...
			$("> div", g.accordion).each(function(i, box) {
				...
				if (jbox.attr("title")) {
					$(".l-accordion-header-inner", header).html(jbox.attr("title"));
					// jbox.attr("title", "");
				}
				if (jbox.attr("icon")) {
					$(".l-accordion-header-inner", header).prepend(UIUtil.iconHTML(jbox.attr("icon")));
				}
				jbox.before(header);
				if (!jbox.hasClass("l-accordion-content"))
					jbox.addClass("l-accordion-content");
			});
			...
			$(">.l-accordion-header", g.accordion).click(function() {
				...
				if (togglebtn.hasClass("l-accordion-toggle-close")) {
					...
					$(this).next(".l-accordion-content").slideDown(p.speed).siblings(".l-accordion-content:visible").slideUp(p.speed);// zzg
																																		// show,hide
																																		// ->
																																		// slide
					...
					$(this).next(".l-accordion-content").slideUp(p.speed);// zzg
																			// show,hide
																			// ->
																			// slide
				}
		...
		onResize : function() {
			...
			var parent = g.accordion.parent();
			if (parent[0].tagName.toLowerCase() == "body") {
				var windowHeight = $(window).height();
				windowHeight -= parseInt(parent.css('paddingTop'));
				windowHeight -= parseInt(parent.css('paddingBottom'));
				g.height = p.heightDiff + windowHeight * parseFloat(p.height) * 0.01;
			} else {
				g.height = p.heightDiff + (parent.height() * parseFloat(p.height) * 0.01);
			}
			g.accordion.height(g.height);
			g.setContentHeight(g.height - g.headerHoldHeight);
		},
		setContentHeight : function(height) {// zzg add
			var g = this;
