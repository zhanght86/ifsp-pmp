	...
	$.ligerui.controls.Panel.ligerExtend($.ligerui.core.UIComponent, {
		...
		toggle : function() {
			...
			if (toggle.hasClass("l-panel-header-toggle-hide")) {
				...
				g.panel.find(".l-panel-content:first").slideDown();
			} else {
				toggle.addClass("l-panel-header-toggle-hide");
				g.panel.find(".l-panel-content:first").slideUp();
			}
		...
		_setUrl : function(url) {
			...
			if (url) {
				...
				setTimeout(function() {
					...
					/*
					 * 可以在子窗口这样使用： var panel = frameElement.panel; var panelData =
					 * dialog.get('data');//获取data参数 panel.set('title','新标题');
					 * //设置标题 panel.close();//关闭dialog
					 */
...
//				g.panel.find('img').remove();
				g.panel.find(".l-panel-header:first i").remove();
			} else {
				g.panel.addClass("l-panel-hasicon");
//				g.panel.append('<img src="' + url + '" />');
				g.panel.find(".l-panel-header:first").prepend(UIUtil.iconHTML(url));
			}
		},
