	...
	l.controls.Dialog
			.ligerExtend(
					l.core.Win,
					{
						...
						_render : function() {
							...
							if (p.target) {
								...
								if (p.load) {
									...
									setTimeout(function() {
										...
										/*
										 * 可以在子窗口这样使用： var dialog =
										 * frameElement.dialog; var dialogData =
										 * dialog.get('data');//获取data参数
							...
							if (p.buttons) {
								$(p.buttons).each(function(i, item) {
									var btn = $('<div ' + (item.id ? "refid=" + item.id : "") + ' class="l-dialog-btn"><div class="l-dialog-btn-l"></div><div class="l-dialog-btn-r"></div><div class="l-dialog-btn-inner"></div></div>');
									$(".l-dialog-btn-inner", btn).html(UIUtil.iconHTML(item.icon) + item.text);// zzg
						...
						updatePosition : function() {// zzg add
							var g = this, p = this.options;
							// 位置初始化
							var left = 0;
							var top = 0;
							p.left = left = 0.5 * ($(window).width() - g.dialog.width());
							p.top = top = 0.5 * ($(window).height() - g.dialog.height()) + $(window).scrollTop() - 10;
							if (left < 0)
								p.left = left = 0;
							if (top < 0)
...
var tips = liger.find('Tip');
						...
						_setHeight : function(value) {
							...
							if (value == "auto") {
								...
								var height = value - this._borderY - g.dialog.buttons.outerHeight() - 8;//
								if (g.trigger('ContentHeightChange', [ height ]) == false)
									return;
						...
						hidden : function() {
							var g = this;
							var abort = g.trigger("beforehide");// zzg
							if (abort === false) {
								return;
							}
							l.win.removeTask(g);
							g.dialog.hide();
							g.unmask();
							g.trigger("afterhide");// zzg
						},
						show : function() {
							var g = this, p = this.options;
							var abort = g.trigger("beforeshow");// zzg
							if (abort === false) {
								return;
							}
							g.mask();
						...
						_saveStatus : function() {
							...
							g._height = g.dialog.body.height() + g._borderY;
							var top = 0;
							var left = 0;
...
var tips = liger.find('Tip');
	...
	$.ligerDialog.waitting = function(message, title) {
		message = message || $.ligerDefaults.Dialog.waittingMessage;
