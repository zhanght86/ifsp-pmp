		...
		this.each(function() {
			...
			if (p.auto || options == undefined) {
				if (!p.content) {
					...
					if (p.removeTitle)
						jtarget.removeAttr("title");
				}
				p.content = p.content || this.title;
				jtarget.bind('mouseover.tip', function() {
				...
				p.x = p.x || 0;
				p.y = p.y || 0;
				var liger = $.ligerTip(p);
				// zzg begin
				var that = this;
				liger.tip.bind('click.tip', function() {
					var tipmanager = $.ligerui.managers[that.ligeruitipid];
					if (tipmanager) {
						tipmanager.remove();
					}
				});
				// zzg end
				// zzg begin fix z-index
				var parentWindow = jtarget.closest(".l-dialog");
				if (parentWindow.length > 0) {
					var zindex = parentWindow.css("z-index") * 1;
					zindex = isNaN(zindex) ? 0 : zindex;
					liger.tip.css("z-index", zindex + 1);
				} else {
					liger.tip.css("z-index", 8000);
				}
				// zzg end
			}
		});
