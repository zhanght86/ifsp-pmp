...
(function($) {
	...
	$.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
		...
		_render : function() {
			...
			// zzg begin
			var isarea = this.element.tagName.toUpperCase() == 'TEXTAREA';
			g.wrapper = g.inputText.wrap('<div class="' + (isarea ? 'l-textarea' : '') + ' l-text"></div>').parent();
			// var className = isarea ? "l-textarea-field" : "l-text-field";
			// if (!isarea)
			if (isarea) {
				g.wrapper.height(Math.max(this.element.rows * 14, 20));
			}
		...
		_setEvent : function() {
			var g = this, p = this.options;
			g.inputText.bind('focusout.textBox', function() {
				g.trigger('blur');
				g.checkNotNull();
				g.checkValue();
				...
				//if (typeof this.oldvalue == "undefined" || this.oldvalue != this.value) {
